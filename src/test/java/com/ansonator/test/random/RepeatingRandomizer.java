...
/**
 * Maintains a cache in memory of all randomly generated objects in order to limit the set of
 * distinct objects that may be created. By default, it will re-generate an object until it has
 * something distinct from all objects previously generated by this Randomizer. Provides method
 * {@link #getAll} for retrieving the full set of distinct objects that can be generated.
 *
 * @author BA030483
 * @param <T>
 */
public abstract class RepeatingRandomizer<T> extends FakerBasedRandomizer<T>
  ...
  /**
   * Used to influence how objects are selected from the pool of previously generated objects. For
   * example, you might want a non-uniform distribution.
   *
   * @param chooser a Function which accepts the current max size for this randomizer and outputs an
   *     integer in the range [0, maxSize).
   */
  ...
  public final T getRandomValue() {
    ...
    if (index < 0 || index >= maxSize) {
      throw new IndexOutOfBoundsException(
          "Chooser must return values in range [0," + maxSize + ")");
    }
