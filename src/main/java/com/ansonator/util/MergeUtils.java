...
public class MergeUtils {
   ...
   *
   * <p>other to our Streamthis stream If the specified key is not already associated with a value
   * or is associated with null, associates it with the given non-null value. Otherwise, replaces
   * the associated value with the results of the given remapping function, or removes if the result
   * is {@code null}. This method may be of use when combining multiple mapped values for a key. For
   ...
   * }</pre>
   *
   * <p>If the remapping function returns {@code null}, the mapping is removed. If the remapping
   ...
   *     from being stored in this map (<a href="{@docRoot}/java.base/java/util/Collection
   *     .html#optional-restrictions">optional</a>)
   ...
   * }</pre>
   *     <p>The default implementation makes no guarantees about detecting if the remapping function
   *     modifies this map during computation and, if appropriate, reporting an error.
   *     Non-concurrent implementations should override this method and, on a best-effort basis,
   *     throw a {@code ConcurrentModificationException} if it is detected that the remapping
   *     function modifies this map during computation. Concurrent implementations should override
   *     this method and, on a best-effort basis, throw an {@code IllegalStateException} if it is
   *     detected that the remapping function modifies this map during computation and as a result
   *     computation would never complete.
   *     <p>The default implementation makes no guarantees about synchronization or atomicity
   ...
   *     subinterface {@link java.util.concurrent.ConcurrentMap} must document whether the remapping
   *     function is applied once atomically only if the value is not present.
   * @since 1.8
   */
  <T, K> Stream<T> concat(
      final Stream<T> left,
      final Stream<T> right,
    ...
    final Stream<T> extra =
        removeNulls(right).filter(t2 -> !leftKeys.contains(extractKey.apply(t2)));
    return Stream.concat(leftJoin, extra);
  }

  <T, K> Stream<? extends T> join(
      final Stream<? extends T> left,
      final Stream<? extends T> right,
      final Function<T, K> extractKey,
      final BiFunction<? super T, ? super T, ? extends T> mergeFn) {
    final Map<K, T> rightMap = removeNulls(right).collect(Collectors.toMap(extractKey, v -> v));
    return removeNulls(left)
        .filter(t -> t != null)
        .map(
            t1 -> {
              K k = extractKey.apply(t1);
