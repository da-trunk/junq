...
/**
 * A wrapped Stream which implements Iterator and Iterable. Useful when you want to remove an
 * element from the stream without terminating it. See {@link #next}.
 *
 * <p>All terminal operations {@link #close} the wrapped Stream before returning.
 *
 * @author BA030483
 * @param <T>
 */
public class PeelingStream<T> implements Stream<T>, Iterator<T>, Iterable<T>, Cloneable {
   ...
   *
   * <p>This clones the stream by materializing it into an in memory-collection. It then uses the
   * collection to recreate two new streams. One is returned as the clone while the other is
   * retained to avoid having the orignal wrapped stream be in a terminal state.
   *
   * <p>So, this consumes twice as much memory as required to represent the original stream. Thus,
   * it should always be called as late as possible in the stream's composition.
   *
   * <p>throws CloneNotSupportedException if the stream's size exceeds MAX_SIZE.
   */
  @Override
  public PeelingStream<T> clone() throws CloneNotSupportedException {
    List<T> list = wrapped.limit(MAX_CLONEABLE_SIZE + 1).collect(Collectors.toList());
    if (list.size() > MAX_CLONEABLE_SIZE) {
      throw new CloneNotSupportedException(
          "Unable to clone a stream of size greater than " + MAX_CLONEABLE_SIZE);
    }
    wrapped = list.stream();
  ...
  ///////////////////// from here, only plain delegate methods but returning our type instead of
  // Stream
